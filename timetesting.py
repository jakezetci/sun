...
def test_dipolelatlon(lattest, lontest, iters=10, dipoleR=600000,
    ...
    error_lon.plot(longitudes[points_lon], mse_array[points_lon], 'o', ms=8,
...
def test_pointlatlon(lattest, lontest, iters=10, dipoleR=600000,
                     latlim=[-30, 30], lonlim=[-30, 30], N=2, M=1e12,
...
def test_borders(sizelims, iters=100, dipoleR=600000,
                 N=2, M=1e12,
                 Rrelative=np.array([10000, -12, 12]), relative=True,
    ...
    if relative:
        fig, error_size = config(None, None,
                                 'grid size in degrees', 'relative error',
                                 'MAPE of grid size', logscaley=True, gapx=1.0,
                                 gapy=0.5)
    else:
        fig, error_size = config(None, None,
                                 'grid size in degrees', 'absolute error',
...
if __name__ == "__main__":
    ...
    latlimits = [-50, 50]
    lonlimits = [-50, 50]
    r1, r2, r3, r4, r5, r6, r7 = test_dipolelatlon(latlimits, lonlimits, iters=500,
                                                   latlim=[-10, 10], lonlim=[-10, 10],
                                                   relative=True)
    ...
    rr1, rr2, rr3, rr4, rr5, rr6, rr7 = test_pointlatlon(l1, l2, iters=500,
                                                         latlim=[-10, 10], lonlim=[-10, 10],
                                                         relative=True, 
                                                         dipolelat=r1, dipolelon=r2)

    sizelims = [0.5, 45]
    rrr1, rrr2, rrr3, rrr4, rrr5, rrr6 = test_borders(sizelims, iters=100,
    ...
    perfsize = [-rrr1, +rrr1]
    N_lims = [0.2, 10]
    t1, t2, t3, t4, t5, t6 = test_density(N_lims,iters=50,
                                          Rrelative=np.array([10000, rr1, rr2]), relative=True,
                                          dipolelat=r1, dipolelon=r2,
                                          latlim=perfsize, lonlim=perfsize,
                                                      timestamps=10)

    print(f'best parameters: dipole coors: {r1:0.2f, r2:0.2f},',
