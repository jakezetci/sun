...
def mp_energy(bitmap_path, magnetogram_path, density=5,
    ...
    if mode == 'track_loc':
        ...
        x, y = np.asarray(vectors).T
        ax2.scatter(x, y, c=points_to_display, alpha=0.6,
...
def create_3Dgrid(hdr, density, cX, cY, mode='default'):
    ...
    xs_unique = xs_unique + d_pixel/3.33333 #shift to avoid nans
    ys_unique = ys_unique + d_pixel/3.33333 #shift
    ...
    if mode == 'default':
        ...
        __x, __y, z = xyR2xyz(-(bitmapcenterX-cX)*d_pixel, -(bitmapcenterY-cY) * d_pixel,
                              r_sun)
        zs_unique = np.linspace(z, z+z_size, num=int(z_num//(density*2)))
    elif mode == 'adaptive':
