...
if __name__ == '__main__':
    ...
    np.savetxt(f'dates_{noaa_ar}_{day}_dens{density}_curv.txt', dates, fmt='%s')
    try:
        energys = np.loadtxt(
    ...
    for i in range(start, N):
        ...
        try:
            ...
            dts = np.loadtxt(f'dates_{noaa_ar}_{day}_dens{density}_curv.txt', dtype=np.datetime64)
            np.savetxt(f'dates_{noaa_ar}_{day}_dens{density}_curv.txt', np.delete(dts, i - failed_counter), fmt='%s')
            failed_counter += 1
            continue

        energy, x, y = computing.mp_energy(bitmap_path, magnetogram_path, density=density,
                                     onlyactive=False, threads=10, mode='fineZ')
        print(f'{date} - {energy}')
